...
class RESTfulAPI(CancelMixin):
    ...
    def serve(self, logging_conf: Optional[dict] = None):
        ...
        # 暂停
        self._router.add_api_route(
            "/v1/models/{model_uid}/pause",
            self.pause_model,
            methods=["DELETE"],
            dependencies=(
                [Security(self._auth_service, scopes=["models:pause"])]
                if self.is_authenticated()
                else None
            ),
        )
        # 恢复
        self._router.add_api_route(
            "/v1/models/{model_uid}/resume",
            self.resume_model,
            methods=["POST"],
            dependencies=(
                [Security(self._auth_service, scopes=["models:resume"])]
                if self.is_authenticated()
                else None
            ),
        )
    ...
    async def list_models(self) -> JSONResponse:
        try:
            supervisor = await self._get_supervisor_ref()

            # 1. 原有 running models
            models = await supervisor.list_models()
            model_list = []
            ...
            # 2. 额外加入 PAUSED models
            instances = await supervisor.get_instance_info(None, None)
            running_ids = set(models.keys())

            for inst in instances:
                if inst["status"] != LaunchStatus.PAUSED.name:
                    continue
                if inst["model_uid"] in running_ids:
                    continue

                spec = None
                try:
                    spec = await supervisor.get_launch_spec(inst["model_uid"])
                except Exception:
                    pass
                if spec:
                ...
                # 构造“伪 model_info”，结构必须对齐
                model_list.append(
                    {
                        "id": inst["model_uid"],
                        "object": "model",
                        "created": inst["instance_created_ts"],
                        "owned_by": "xinference",
                        "model_uid": inst["model_uid"],
    ...
    # 暂停、恢复
    async def pause_model(self, model_uid: str) -> JSONResponse:
        supervisor = await self._get_supervisor_ref()

        infos = await supervisor.get_instance_info(
            model_name=None,
            model_uid=model_uid,
        )
        if not infos:
        ...
        instance = infos[0]
        if instance["status"] != LaunchStatus.READY.name:
            raise HTTPException(400, "model not ready")

        await supervisor.pause_model_resources(model_uid)
    ...
    async def resume_model(self, model_uid: str) -> JSONResponse:
        supervisor = await self._get_supervisor_ref()
        ...
        instance = infos[0]
        if instance["status"] != LaunchStatus.PAUSED.name:
            raise HTTPException(400, "model not paused")

        await supervisor.resume_model_resources(model_uid)
    ...
    async def terminate_model(self, model_uid: str) -> JSONResponse:
        try:
            ...
            if status == LaunchStatus.PAUSED.name:
                await supervisor.delete_paused_model(model_uid)

            elif status == LaunchStatus.READY.name:
                await supervisor.terminate_model(model_uid)

            else:
                raise HTTPException(
                    400, f"model status {status} cannot be deleted"
                )
