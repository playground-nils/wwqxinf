...
class SupervisorActor(xo.StatelessActor):
    def __init__(self):
        ...
        self._paused_model_specs: Dict[str, Dict] = {}
    ...
    async def launch_builtin_model(
        ...
        **kwargs,
        ...
        # ===============================
        # Save launch spec for pause/resume
        self._launch_specs[model_uid] = {
            "model_uid": model_uid,
    ...
    # ================================
    async def get_launch_spec(self, model_uid: str):
        return self._launch_specs.get(model_uid)

    async def pause_model_resources(self, model_uid: str):
        replica_info = self._model_uid_to_replica_info.get(model_uid)
        if replica_info is None:
            raise ValueError(f"Model not found, uid: {model_uid}")

        self._paused_model_specs[model_uid] = self._launch_specs.get(model_uid)

        for rep_model_uid in iter_replica_model_uid(
                model_uid, replica_info.replica
        ):
            worker_refs = self._replica_model_uid_to_worker.get(rep_model_uid)
            if not worker_refs:
                continue
            ...
            for worker_ref in worker_refs:
                await worker_ref.terminate_model(model_uid=rep_model_uid)

            self._replica_model_uid_to_worker.pop(rep_model_uid, None)

            replica_id = int(rep_model_uid.rsplit("-", 1)[-1])
            await self._status_guard_ref.update_replica_status(
                model_uid,
                replica_id,
                {
                    "status": LaunchStatus.PAUSED.name,
                    "worker_address": None,
                    "error_message": None,
                },
            )
    ...
    async def delete_paused_model(self, model_uid: str):
        self._model_uid_to_replica_info.pop(model_uid, None)
        self._launch_specs.pop(model_uid, None)
        self._paused_model_specs.pop(model_uid, None)
    ...
    async def resume_model_resources(self, model_uid: str):

        spec = self._paused_model_specs.get(model_uid)
        if not spec:
            raise ValueError(f"Model {model_uid} is not paused or spec missing")

        infos = await self._status_guard_ref.get_instance_info(model_uid=model_uid)
        if not infos or infos[0].status != LaunchStatus.PAUSED.name:
            raise ValueError("Model is not paused")

        self._model_uid_to_replica_info.pop(model_uid, None)

        await self.launch_builtin_model(**spec)
