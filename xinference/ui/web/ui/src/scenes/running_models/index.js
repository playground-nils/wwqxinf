  ...
  const llmColumns = [
    ...
    replicaColumn,
    {
      ...
      renderCell: ({ row }) => {
        ...
        {/*===========*/}
        const pauseUrl = `${endPoint}/v1/models/${row.id}/pause`
        const resumeUrl = `${endPoint}/v1/models/${row.id}/resume`
          ...
          >
            ...
            <IconButton
              ...
              onClick={async () => {
                ...
                try {
                  setIsCallingApi(true)
                  const res = await fetcher(pauseUrl, {
                    method: 'DELETE',
                  })
                  if (!res.ok) {
                  ...
                  console.error('pause model failed:', err)
                } finally {
                  setIsCallingApi(false)
                }
              }}
            ...
            <IconButton
              ...
              onClick={async () => {
                ...
                try {
                  setIsCallingApi(true)
                  const res = await fetcher(resumeUrl, {
                    method: 'POST',
                  })
                  if (!res.ok) {
                    const msg = await res.text()
                  ...
                  console.error('resume model failed:', err)
                } finally {
                  setIsCallingApi(false)
                }
              }}
